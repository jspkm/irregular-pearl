              ...
              <tr key={u.id} className="border-b border-border/50 last:border-b-0">
                ...
                <td className="px-4 py-2">
                  <button
                    ...
                    className={`text-[10px] px-2 py-0.5 rounded-full border-none cursor-pointer ${u.is_maestro ? 'bg-accent-soft text-accent font-medium' : 'bg-gray-100 text-gray-400'}`}
                  >
                  ...
                  {u.role === 'moderator' && u.managed_sections?.length > 0 ? (
                    <div className="flex flex-wrap gap-1">
                      {u.managed_sections.map(s => (
                        <span key={s} className="text-[10px] bg-accent-soft text-accent px-1.5 py-0.5 rounded-full">{s}</span>
                      ))}
                    </div>
                  ) : '—'}
                </td>
        ...
        <div className="fixed inset-0 bg-black/30 flex items-center justify-center z-50">
          <div className="bg-surface border border-border rounded-xl p-6 w-96 max-h-[80vh] overflow-y-auto">
            ...
            <div className="flex gap-2 justify-end">
              ...
              <button onClick={() => saveSections(editingSections)} className="text-sm text-bg bg-ink px-4 py-1.5 rounded-lg border-none cursor-pointer">Save</button>
            </div>
          </div>
        </div>
      )}
