    ...
    <div>
      <div className="flex gap-3 mb-6">
        <input
          ...
          className="flex-1 border border-border rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-[#6B4E7C]"
        />
        <select
          value={roleFilter}
              ...
              <tr key={u.id} className="border-b border-border/50 last:border-b-0">
                ...
                <td className="px-4 py-2">
                  <button
                    ...
                    className={`text-[10px] px-2 py-0.5 rounded-full border-none cursor-pointer ${u.is_maestro ? 'bg-[#F2EEF5] text-[#6B4E7C] font-medium' : 'bg-gray-100 text-gray-400'}`}
                  >
        ...
        <div className="fixed inset-0 bg-black/30 flex items-center justify-center z-50">
          <div className="bg-surface border border-border rounded-xl p-6 w-96 max-h-[80vh] overflow-y-auto">
                ...
                return (
                  <button
                    ...
                    className={`text-xs px-3 py-1.5 rounded-full border cursor-pointer transition-colors capitalize ${
                      selected
                        ? 'bg-[#F2EEF5] text-[#6B4E7C] border-[#6B4E7C] font-medium'
                        : 'bg-white text-muted border-border hover:border-[#6B4E7C] hover:text-[#6B4E7C]'
                    }`}
                  >
                    {s}
                  </button>
            ...
            <div className="flex gap-2 justify-end">
              ...
              <button onClick={() => saveSections(editingSections)} className="text-sm text-white bg-[#1A1A1A] px-4 py-1.5 rounded-lg border-none cursor-pointer">Save</button>
            </div>
          </div>
        </div>
      )}
